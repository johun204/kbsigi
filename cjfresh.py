...
def _buildQuickReplies(location_nm, dataName):
	quickReplies = []
	if location_nm == '전경련회관':
		quickReplies.append({'label': '오늘', 'action': 'message', 'messageText': f'오늘 {location_nm} {dataName} 메뉴 알려줘~!'})
		quickReplies.append({'label': '내일', 'action': 'message', 'messageText': f'내일 {location_nm} {dataName} 메뉴 알려줘~!'})
		quickReplies.append({'label': '모레', 'action': 'message', 'messageText': f'모레 {location_nm} {dataName} 메뉴 알려줘~!'})
		...
		quickReplies.append({'label': '처음으로', 'action': 'message', 'messageText': '처음으로'})
	quickReplies.append({'label': '큽식이 추천(AI)', 'action': 'message', 'messageText': f'오늘 {location_nm} 큽식이(AI) 추천메뉴 뭐야~?'})
	return quickReplies
...
def getFmenu(req_dt, storeIdx, txt=''):
	...
	dataType, dataName = _resolveMealType(txt if storeIdx == '6083' else '')
