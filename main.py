...
def main():
	...
	# cjfresh (전경련회관)
	cjfresh_1a_today = getFmenu(today_date, '6083', '아침')
	cjfresh_1b_today = getFmenu(today_date, '6083', '점심')
	cjfresh_1c_today = getFmenu(today_date, '6083', '저녁')
	save_with_error_policy('data/cjfresh_1a.json', cjfresh_1a_today, now_kst)
	save_with_error_policy('data/cjfresh_1b.json', cjfresh_1b_today, now_kst)
	save_with_error_policy('data/cjfresh_1c.json', cjfresh_1c_today, now_kst)
	save_with_error_policy('data/cjfresh_1a_after1.json', getFmenu(after1_date, '6083', '아침'), now_kst)
	save_with_error_policy('data/cjfresh_1b_after1.json', getFmenu(after1_date, '6083', '점심'), now_kst)
	save_with_error_policy('data/cjfresh_1c_after1.json', getFmenu(after1_date, '6083', '저녁'), now_kst)
	save_with_error_policy('data/cjfresh_1a_after2.json', getFmenu(after2_date, '6083', '아침'), now_kst)
	save_with_error_policy('data/cjfresh_1b_after2.json', getFmenu(after2_date, '6083', '점심'), now_kst)
	save_with_error_policy('data/cjfresh_1c_after2.json', getFmenu(after2_date, '6083', '저녁'), now_kst)

	# cjfresh (한국교직원공제회)
	cjfresh_2b_today = getFmenu(today_date, '6848', '점심')
	save_with_error_policy('data/cjfresh_2b.json', cjfresh_2b_today, now_kst)
	save_with_error_policy('data/cjfresh_2b_after1.json', getFmenu(after1_date, '6848', '점심'), now_kst)
	save_with_error_policy('data/cjfresh_2b_after2.json', getFmenu(after2_date, '6848', '점심'), now_kst)
	# -------------------- README.md 생성 (오늘자 메뉴 표) -------------------- #
	readme_entries = [
		('여의도IT센터', hcafeteria_today),
		('국민은행 신관', ourhome1_today),
		('국민은행 본점', ourhome2_today),
		('전경련회관', cjfresh_1b_today),
		('한국교직원공제회', cjfresh_2b_today),
	]
